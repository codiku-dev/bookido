...
export function PublicCoachBanner(p: { name: string; bio: string | null; imageUrl: string | null; compact?: boolean }) {
  ...
  const mobileTextBlock = (
    <div className="min-w-0 flex-1 text-left">
      <p className="text-[10px] font-semibold uppercase tracking-wider text-white/85">{t("public.booking.coachSectionLabel")}</p>
  ...
  const mobileBanner = (
    <div className="relative mb-5 overflow-hidden rounded-3xl bg-gradient-to-br from-blue-600 via-blue-600 to-cyan-500 p-4 shadow-lg ring-1 ring-blue-500/30 md:hidden">
      <div className="pointer-events-none absolute -right-6 top-0 h-32 w-32 rounded-full bg-white/15 blur-2xl" aria-hidden />
      <div className="pointer-events-none absolute -bottom-8 -left-4 h-28 w-28 rounded-full bg-cyan-300/25 blur-2xl" aria-hidden />
